...
% -----------------------------------------------------------------------------
% ABSTRACT (Elemento obrigatório para trabalhos em português)
% -----------------------------------------------------------------------------
\newpage
\begin{center}
...
\noindent
This work applies Linear Programming techniques to optimize the allocation of public security resources among the 27 Brazilian states. Using real data from the Atlas of Violence (IPEA/FBSP) and the Brazilian Yearbook of Public Security, we developed a mathematical model that determines the optimal distribution of a supplementary budget to minimize the number of violent deaths. The model considers the crime-investment elasticity of each state, calculated by regression over 34 years of historical data (1989-2022). Results indicate that, with an additional investment of R\$ 5 billion distributed optimally, it would be possible to save approximately 1,875 lives per year. Furthermore, we identified that states such as São Paulo, Santa Catarina, and Minas Gerais show greater efficiency in resource use, while Bahia, Sergipe, and Amapá need management improvements. The work includes sensitivity analysis, Monte Carlo simulation, and validation through historical backtesting.\\[0.5cm]

\noindent
\textbf{Keywords:} Operations Research. Linear Programming. Public Security. Optimization. Simplex.
